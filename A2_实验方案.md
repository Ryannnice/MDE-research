...
>
> **§8–14 是执行层(2026-06-23 新增):显卡服务器可直接接手。** 七个脚本全部本地 `_self_check` 跑通(无 GPU/权重),GPU 服务器换数据集与权重即真跑。脚本清单见 §8.1,逐 Phase 命令见 §11,目录结构见 §8.2。
...
8. **🔴 第二轮复核:修复在相邻处留的 4 个缺陷(2026-06-23,同一审稿视角复核修复 diff)**——B1/B2 病灶虽消,但修复各自在旁边留了 major,继续留痕:
   ...
   - **#4 minor**:skeleton docstring model_id 对齐 E2E-FT Day-1 起点;`requirements_A2.txt` 钉 `diffusers>=0.32,<0.40`(接口按 v0.38 核实,防未来版漂移);失败切片距离图用 `scipy.ndimage.distance_transform_edt` 替纯 Python 双循环。
9. **🔴 第三轮复核:又抓一个 blocker(B3)+ 一个 frame 修正(2026-06-23,同一审稿视角验修复 diff)**——前两轮修复在相邻处又暴露问题,继续留痕:
   ...
   - **复核确认**:三轮审查后 8/8 自检通过,无残留 blocker;显卡服务器可接手。**接手第一件盯的事**:真机首跑 L1,在 arm C 上 print min/max,**必须落米制量程(NYU 0-10m)而非 [0,1]**——这是 B3 是否真生效的一秒级证伪点,优先级高于 NYU 轴序、VAE 残差。
...
### 8.1 七个脚本(全部本地 `_self_check` 跑通,无 GPU/权重)
...
> 验证全部自检一条命令:`for f in A2_ccf_depth_skeleton A2_geo_anchor A2_eval_protocol A2_baselines_postproc A2_diag_bias_var A2_marigold_bridge A2_failure_slices A2_run_grid; do python $f.py; done`
> 环境:`pip install -r requirements_A2.txt`(torch 按服务器 CUDA 另装)。
...
| 臂 | driver 名 | 基底 | 接地 | 守哪侧 |
|----|-----------|------|------|--------|
| A | `A_postproc_global` | 单步 | 全局 scale+shift(2-DOF) | 下界 |
| B | `B_postproc_patch` | 单步 | patch-affine(AnchorD 式) | 便宜基线 |
| **B'** | `Bp_postproc_patch_ccf` | **CCF(同 C)** | patch-affine 后处理 | **单变量门槛** |
| C | `C_sample_geo_local` | CCF | 采样期注入(本工作) | — |
| D | `D_sample_geo_global` | CCF | = C 但 patch 网格退全局 | 消融"局部 patch" |
...
- **🔴 两组对照(round-2 #2 修复,审稿复核发现 base predictor 混淆)**:
  - **C vs B'(单变量门槛)**:B' 与 C **同 CCF 基底**,唯一差异 = 几何锚施加位置(采样期烘焙+往返传播 vs 解码后一次性后处理)。`AbsRel_metric(C) < AbsRel_metric(B')` 才**干净证明**"采样期注入 > 后处理"(§4.8 Linchpin-1 严谨判据)。
...
for f in A2_ccf_depth_skeleton A2_geo_anchor A2_eval_protocol \
         A2_baselines_postproc A2_diag_bias_var A2_marigold_bridge A2_run_grid; do
...
`results.csv` 字段:`backbone,dataset,phase,arm,sample_id,seed,K,nfe,protocol,align,absrel,rmse,log10,d1,d2,d3,align_gain`。
...
# Linchpin-1 判据:C 是否在小 K 优于 B
print(g.loc["C_sample_geo_local"] - g.loc["B_postproc_patch"])   # <0 即 C 赢
```

## 13. 已知坑(Phase 0 先排,立项卡 §6)

- **🔴🔴 接手第一件盯的事(B3,一秒级证伪点)**:真机首跑 L1,在 arm C 上 `print(predC.min(), predC.max())` —— **必须落米制量程(NYU 0–10m),不能是 [0,1]**。arm C 末步 `decode_depth` 出相对深度,必须经 `ground_to_metric` 接地才能与 metric 协议 GT 同框(round-3 已修 + 加回归断言)。若仍见 [0,1],则 L1 主表全废、且会得"采样期注入=负贡献"的反向错论。这是比 NYU 轴序、VAE 残差都靠前的检查点。
...
**文档**
- `A2_实验方案.md`(本文件:§0–7 决策序列 + §8–14 执行层)
- `requirements_A2.txt`(GPU 服务器依赖)

**脚本(七个,全部 `_self_check` 跑通)**
...
**GPU 服务器接手清单**(改三处即真跑):
1. `pip install -r requirements_A2.txt` + torch(按 CUDA)
2. 下载 `data/nyu/...`(§10 序 1,无注册)
3. 冒烟:`python A2_run_grid.py --phase L1 --mock`(无 GPU 验证流水线;输出标 `backbone=MOCK`)
4. 真跑:`python A2_run_grid.py --phase L1 --backbone e2eft --dataset nyu --root data/nyu --K 1 2 4 8 --seeds 0 1 2 --out runs/L1_nyu.csv`
